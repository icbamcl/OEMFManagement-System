...
namespace AMCL.BL
{
    public class UnitRepurchaseBL
    {
        ...
        public void saveRepurchase(DataTable dtTransferSaleCert, UnitHolderRegistration regObj, UnitRepurchase repObj, UnitUser unitUserObj)
        {
            try
            {
                ...
                for (int loop = 0; loop < dtTransferSaleCert.Rows.Count; loop++)
                {
                    ...
                    else if (dtTransferSaleCert.Rows[loop]["SL_NO"].ToString().ToUpper().IndexOf('D') >= 0)
                    {
                        htRepurchaseRegInfo = new Hashtable();
                        htRepurchaseRegInfo.Add("CURR_REG_BK", regObj.FundCode.ToString().ToUpper());
                        htRepurchaseRegInfo.Add("CURR_REG_BR", regObj.BranchCode.ToString().ToUpper());
                        ...
                        saleNoArray = dtTransferSaleCert.Rows[loop]["SL_NO"].ToString().Split('D');
                        SL_TR_RN = Convert.ToInt32(saleNoArray[1].ToString());
                        certArray = dtTransferSaleCert.Rows[loop]["CERTIFICATE"].ToString().Split('-');
                        CertNo = Convert.ToInt32(certArray[1].ToString());
                        certType = certArray[0].ToString().ToUpper();
                        ...
                        Old_SL_TR_RN = "D" + SL_TR_RN.ToString();
        ...
        public void saveRepurchaseCDS(DataTable dtSaleTrInfo, UnitHolderRegistration regObj, UnitRepurchase repObj, UnitUser unitUserObj)
        {
            try
            {
                DataTable dtSaleForRepurchaseSummary = dtSaleTrInfo;
                ...
                for (int loop = 0; loop < dtSaleTrInfo.Rows.Count; loop++)
                {
                    if (dtSaleTrInfo.Rows[loop]["SL_TR_NO"].ToString().ToUpper().IndexOf('S') >= 0)
                    {
                        ...
                        if (regObj.BranchCode.ToString().ToUpper() != "AMC/01")
                        {
                            ...
                            //}
                            //else
                            if (repObj.PayType.ToString().ToUpper() == "CHQ")
                            {
                                htRepurchaseRegInfo.Add("CHEQUE_DATE", repObj.RepurchaseDate);
                                htRepurchaseRegInfo.Add("CHEQUE_NO", "BR_STATEMENT");
                    ...
                    else if (dtSaleTrInfo.Rows[loop]["SL_TR_NO"].ToString().ToUpper().IndexOf('T') >= 0)
                    {
                        ...
                        sbQuery.Append(" AND REG_NO_I=" + Convert.ToInt32(regObj.RegNumber.ToString()) + " AND TR_TYPE='T' AND TR_NO=" + Convert.ToInt32(saleNoArray[1].ToString()));
                    ...
                    else if (dtSaleTrInfo.Rows[loop]["SL_TR_NO"].ToString().ToUpper().IndexOf('D') >= 0)
                    {

                        sbQuery = new StringBuilder();
                        saleNoArray = dtSaleTrInfo.Rows[loop]["SL_TR_NO"].ToString().Split('D');
                        sbQuery.Append("UPDATE TRANSFER SET QTY_OUT=NVL(QTY_OUT,0)+" + Convert.ToInt64(dtSaleTrInfo.Rows[loop]["SURRENDER_UNITS"].ToString()));
                        sbQuery.Append(" WHERE F_CD='" + regObj.FundCode.ToString().ToUpper() + "' AND BR_CODE='" + regObj.FundCode.ToString() + "_" + regObj.BranchCode.ToString().ToUpper() + "'");
                        sbQuery.Append(" AND REG_NO_I=" + Convert.ToInt32(regObj.RegNumber.ToString()) + " AND TR_TYPE='D'  AND TR_NO=" + Convert.ToInt32(saleNoArray[1].ToString()));
                ...
                DataTable dtSummaryRepurchase = new DataTable();
                //if (dtSaleForRepurchaseSummary.Rows.Count > 1)
                //    dtSummaryRepurchase = dtSummaryRepurchaseCDS(dtSaleForRepurchaseSummary);
                //else
                    dtSummaryRepurchase = dtSaleTrInfo;



                for (int i = 0; i < dtSummaryRepurchase.Rows.Count; i++)
                {
                    htRepurchaseRegInfo = new Hashtable();
                    htRepurchaseRegInfo.Add("REP_NO", repObj.RepurchaseNo);
                    htRepurchaseRegInfo.Add("REP_DT", repObj.RepurchaseDate);
                    htRepurchaseRegInfo.Add("SL_TR_NO", dtSaleTrInfo.Rows[i]["SL_TR_NO"]);
                    htRepurchaseRegInfo.Add("REG_BK", regObj.FundCode.ToString().ToUpper());
                    htRepurchaseRegInfo.Add("REG_BR", regObj.BranchCode.ToString().ToUpper());
                    if (regObj.BranchCode.ToString().ToUpper() != "AMC/01")
                    {
                    ...
                    htRepurchaseRegInfo.Add("REG_NO", regObj.RegNumber.ToString());
                    htRepurchaseRegInfo.Add("REP_PRICE", repObj.RepurchaseRate);
                    htRepurchaseRegInfo.Add("QTY", dtSaleTrInfo.Rows[i]["SURRENDER_UNITS"]);
                    htRepurchaseRegInfo.Add("PAY_TYPE", repObj.PayType.ToString().ToUpper());
                    htRepurchaseRegInfo.Add("USER_NM", unitUserObj.UserID.ToString());
                    htRepurchaseRegInfo.Add("ENT_DT", DateTime.Now.ToString());
                    htRepurchaseRegInfo.Add("ENT_TM", DateTime.Now.ToShortTimeString().ToString());
        ...
        public string undeliverdCert(string fund_code, string certificate)
        {
            string undeliverdCert = "";
            string[] CertList = certificate.Split('-');
            DataTable dtSaleCertInfo = commonGatewayObj.Select(" SELECT SL_NO FROM SALE WHERE SL_NO IN (SELECT SL_NO FROM SALE_CERT WHERE CERT_NO=" + CertList[0] + " AND CERT_TYPE='" + CertList[1].ToUpper() + "' AND REG_BK='" + fund_code + "') AND CERT_DLVRY_DT IS NULL");
            if (dtSaleCertInfo.Rows.Count>0)
            {
                undeliverdCert = undeliverdCert;
            }

                return undeliverdCert;
        }
        public DataTable dtSummaryRepurchaseCDS(DataTable dtSale)
        {
            DataTable dtSearTable = new DataTable();
            dtSearTable = dtSale;
            dtSearTable.TableName = "Search";
            DataTable dtTarget = new DataTable();
            dtTarget = dtSearchTable(dtSale);
            dtTarget.TableName = "Target";
            int qty = 0;

            DataTable dtRepurchase = new DataTable();
            dtRepurchase.Columns.Add("SL_TR_NO", typeof(string));
            dtRepurchase.Columns.Add("QTY", typeof(string));
          
         
            DataRow drRepurchase;
            ...
            for (int loop = 0; loop < dtTarget.Rows.Count; loop++)
            {
                for (int looper = 0; looper < dtSearTable.Rows.Count; looper++)
                {
                    ...
                    if (dtTarget.Rows[loop]["SALE_TR_NO"].ToString().ToUpper() == dtSearTable.Rows[looper]["SL_TR_NO"].ToString().ToUpper())
                    {                      
                        qty = qty + Convert.ToInt32(dtSearTable.Rows[looper]["SURRENDER_UNITS"].ToString());
                        rowIndex.Add(looper);
                        flag = true;
                ...
                if (flag)
                {
                    drRepurchase = dtRepurchase.NewRow();
                    drRepurchase["QTY"] = qty;
                    drRepurchase["SL_TR_NO"] = dtTarget.Rows[loop]["SALE_TR_NO"].ToString().ToUpper();
                    dtRepurchase.Rows.Add(drRepurchase);
        ...
        public DataTable dtSearchTable(DataTable dtTarget)
        {
            ...
            dtTargetTable.Columns.Add("SALE_TR_NO", typeof(string));
            dtTargetTable.Columns.Add("SURUNITS", typeof(string));
            ...
            for (int loop = 0; loop < dtTarget.Rows.Count; loop++)
            {
                ...
                drTargetTable["SALE_TR_NO"] = dtTarget.Rows[loop]["SL_TR_NO"].ToString();
                drTargetTable["SURUNITS"] = dtTarget.Rows[loop]["SURRENDER_UNITS"].ToString();
                drTargetTable["QTY"] = dtTarget.Rows[loop]["EXIST_UNITS"].ToString();
        ...
        public string SIP_ACC_FRAC_LIB_CODE(string fund_Code)
        {
            DataTable dtUnitBankPaymentCode = commonGatewayObj.Select("SELECT SIP_ACC_FRAC_LIB_CODE FROM FUND_INFO WHERE FUND_CD='" + fund_Code + "'");
            string unitBankPaymentCode = dtUnitBankPaymentCode.Rows[0]["SIP_ACC_FRAC_LIB_CODE"].ToString();
            return unitBankPaymentCode;
        }
        ...
        public DataTable dtGetBEFTNAdviceData(string filter)
        {
            ...
            sbQuery.Append(" C.REG_BK || '/' || C.REG_BR || '/' || C.REG_NO || ',RP-' || B.REP_NO AS REMARKS, B.VOUCHER_NO, B.BEFTN_DATE, C.HNAME,CASE  WHEN LENGTH( B.HOLDER_AC_NO)>13 THEN SUBSTR( B.HOLDER_AC_NO, - 13) ELSE LPAD( B.HOLDER_AC_NO,13,'0') END AS HOLDER_AC_NO, B.HOLDER_ROUTING_NO, B.TOTAL_AMOUNT, A.BANK_CODE");
        ...
        public DataTable dtGetChequePrintData(string filter)
        {
            StringBuilder sbQuery = new StringBuilder();
            sbQuery.Append(" SELECT A.VOUCHER_NO, FUND_INFO.FUND_NM,FUND_INFO.BANK_CODE, U_MASTER.HNAME,A.AUDITED_BY, A.REP_NO, A.REG_BK, A.REG_BR, A.REG_NO, A.REP_DT, A.REP_PRICE, A.QTY, A.TOTAL,");
            sbQuery.Append(" A.CHEQUE_NO,  A.CHEQUE_DATE ");
            sbQuery.Append(" FROM (SELECT CHEQUE_NO, TO_CHAR(CHEQUE_DATE, 'DD-MON-YYYY') AS CHEQUE_DATE,VOUCHER_NO, AUDITED_BY,REP_NO, REG_BK, REG_BR, REG_NO, TO_CHAR(REP_DT, 'DD-MON-YYYY') AS REP_DT, REP_PRICE, SUM(QTY) AS QTY, SUM(REP_PRICE * QTY) AS TOTAL ");
            sbQuery.Append(" FROM REPURCHASE WHERE (PAY_TYPE = 'CHQ') AND (CHEQUE_DATE IS NOT NULL)  AND (CHEQUE_NO IS NOT NULL)  AND (CHQ_PRINT_DT IS  NULL)");
            sbQuery.Append(" GROUP BY VOUCHER_NO,AUDITED_BY, REP_NO, REG_BK, REG_BR, REG_NO, REP_DT, REP_PRICE,CHEQUE_NO,CHEQUE_DATE) A ");
            sbQuery.Append(" INNER JOIN   U_MASTER ON A.REG_BK = U_MASTER.REG_BK AND A.REG_BR = U_MASTER.REG_BR AND A.REG_NO = U_MASTER.REG_NO INNER JOIN ");
            sbQuery.Append(" FUND_INFO ON A.REG_BK = FUND_INFO.FUND_CD  ");
            sbQuery.Append(" WHERE 1=1" + filter.ToString());
            sbQuery.Append(" ORDER BY A.REG_BK, A.REG_BR, A.REP_NO ");
        ...
        public bool IsValidMobileNumber(UnitHolderRegistration regObj)
        {
            ...
            DataTable dtValidMobile = commonGatewayObj.Select("SELECT * FROM U_MASTER WHERE LENGTH(MOBILE1) = 11 AND MOBILE1 NOT LIKE '%-%' AND MOBILE1 NOT LIKE '011%' AND  U_MASTER.REG_BK='" + regObj.FundCode.ToString().ToUpper() + "' AND U_MASTER.REG_BR='" + regObj.BranchCode.ToString() + "' AND U_MASTER.REG_NO=" + regObj.RegNumber);
        ...
        public DataTable dtSMSList(string  type, string fund_code)
        {
            StringBuilder sbQuery = new StringBuilder();
            if(type.ToString()=="0" && fund_code.ToString() == "0")
            {
                sbQuery.Append(" SELECT DISTINCT U_MASTER.REG_BK, U_MASTER.REG_BR, U_MASTER.REG_NO,U_MASTER.HNAME, U_MASTER.MOBILE1,REPURCHASE.REP_NO AS TRAN_NO,TO_CHAR(REPURCHASE.SMS_CREATE_DATE,'DD-MON-YYYY') AS SMS_CREATE_DATE, ");
                sbQuery.Append(" 'Dear Sir, Please Collect Unit Repurchase Cheque of '|| FUND_INFO.FUND_NM||' from our office. For info: 028300412. '|| chr(10) || ' Regards, ICB AMCL.' AS SMS, 'CHQ' AS SMS_TYPE ");
                sbQuery.Append(" FROM U_MASTER, FUND_INFO, REPURCHASE  WHERE  U_MASTER.REG_BK = FUND_INFO.FUND_CD AND U_MASTER.REG_BK = REPURCHASE.REG_BK AND U_MASTER.REG_BR = REPURCHASE.REG_BR ");
                sbQuery.Append(" AND U_MASTER.REG_NO = REPURCHASE.REG_NO AND REPURCHASE.SMS_CREATE_DATE IS NOT NULL AND  REPURCHASE.SMS_SENT_DATE IS NULL AND REPURCHASE.PAY_TYPE='CHQ' ");
            }
            else if (type.ToString() == "0" && fund_code.ToString() != "0")
            {
                sbQuery.Append(" SELECT DISTINCT U_MASTER.REG_BK, U_MASTER.REG_BR, U_MASTER.REG_NO,U_MASTER.HNAME, U_MASTER.MOBILE1,REPURCHASE.REP_NO AS TRAN_NO, TO_CHAR(REPURCHASE.SMS_CREATE_DATE,'DD-MON-YYYY') AS SMS_CREATE_DATE, ");
                sbQuery.Append(" 'Dear Sir, Please Collect Unit Repurchase Cheque of '|| FUND_INFO.FUND_NM||' from our office. For info: 028300412. '|| chr(10) || ' Regards, ICB AMCL.' AS SMS, 'CHQ' AS SMS_TYPE ");
                sbQuery.Append(" FROM U_MASTER, FUND_INFO, REPURCHASE  WHERE  U_MASTER.REG_BK = FUND_INFO.FUND_CD AND U_MASTER.REG_BK = REPURCHASE.REG_BK AND U_MASTER.REG_BR = REPURCHASE.REG_BR ");
                sbQuery.Append(" AND U_MASTER.REG_NO = REPURCHASE.REG_NO AND REPURCHASE.SMS_CREATE_DATE IS NOT NULL AND  REPURCHASE.SMS_SENT_DATE IS NULL AND REPURCHASE.PAY_TYPE='CHQ' ");
                sbQuery.Append(" AND  U_MASTER.REG_BK='" + fund_code.ToString() + "' AND REPURCHASE.REG_BK ='" + fund_code.ToString() + "' ");
            }

                DataTable dtSMSList = commonGatewayObj.Select(sbQuery.ToString());
            return dtSMSList;
