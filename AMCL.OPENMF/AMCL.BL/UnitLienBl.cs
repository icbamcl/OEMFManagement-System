...
namespace AMCL.BL
{
    public class UnitLienBl
    {
        ...
        public long totalLienAmountWithDate(UnitHolderRegistration unitRegObj, string transDate)
        {
            long totalLien = 0;
            StringBuilder sbQuery = new StringBuilder();
            sbQuery.Append("SELECT SUM(QTY) AS TOTAL_LIEN FROM LIEN_MARK_CERT_NO WHERE LN_MK_CANCEL IS NULL AND REG_BR='" + unitRegObj.BranchCode.ToString() + "'");
            sbQuery.Append(" AND REG_BK='" + unitRegObj.FundCode.ToString() + "' AND REG_NO=" + unitRegObj.RegNumber+" " );
            sbQuery.Append(" AND LIEN_NO IN (SELECT LIEN_NO FROM LIEN_MARK WHERE REG_BR='" + unitRegObj.BranchCode.ToString() + "' AND REG_BK='" + unitRegObj.FundCode.ToString() + "' AND REG_NO=" + unitRegObj.RegNumber + " AND LIEN_DT<='"+ transDate + "' ) ");
