...
namespace AMCL.DL
{
    public class OMFDAO
    {
        ...
        public DataTable getDtTotalSaleUnitCerts(UnitHolderRegistration regObj)
        {
            ...
            sbQueryString.Append(" SELECT 'T' || TR_NO AS SL_NO, CERTIFICATE, QTY FROM   TRANS_CERT WHERE (STATUS_FLAG IS NULL) AND (VALID IS NULL) AND (TR_TYPE='T')  AND (BR_CODE = '" + regObj.FundCode.ToString() + "_" + regObj.BranchCode.ToString() + "') AND (TR_NO IN");
            ...
            sbQueryString.Append(" (REG_BR_I = '" + regObj.BranchCode.ToString() + "') AND (TR_TYPE='T') AND (VALID IS NULL))) UNION ALL");

            sbQueryString.Append(" SELECT 'D' || TR_NO AS SL_NO, CERTIFICATE, QTY FROM   TRANS_CERT WHERE (STATUS_FLAG IS NULL) AND (VALID IS NULL) AND (TR_TYPE='D')  AND (BR_CODE = '" + regObj.FundCode.ToString() + "_" + regObj.BranchCode.ToString() + "') AND (TR_NO IN");
            sbQueryString.Append(" (SELECT TR_NO FROM  TRANSFER WHERE (REG_BK_I = '" + regObj.FundCode.ToString() + "') AND (BR_CODE = '" + regObj.FundCode.ToString() + "_" + regObj.BranchCode.ToString() + "')AND (REG_NO_I = " + regObj.RegNumber.ToString() + ") AND ");
            sbQueryString.Append(" (REG_BR_I = '" + regObj.BranchCode.ToString() + "') AND (TR_TYPE='D') AND (VALID IS NULL))) UNION ALL");
        ...
        public DataTable getDtTotalSaleUnitCertsCDS(UnitHolderRegistration regObj)
        {
            ...
            sbQueryString.Append(" WHERE QTY>NVL(QTY_OUT,0)  AND (TR_TYPE='T') AND (REG_BK_I = '" + regObj.FundCode.ToString() + "') AND (REG_BR_I = '" + regObj.BranchCode.ToString() + "') AND (REG_NO_I = " + regObj.RegNumber.ToString() + ") ");

            sbQueryString.Append(" UNION ALL ");
            sbQueryString.Append(" SELECT 'D' || TR_NO AS SL_TR_NO, QTY - NVL(QTY_OUT, 0) AS SURRENDER_UNITS, NVL(QTY_OUT, 0) AS OUT_UNITS, QTY - NVL(QTY_OUT, 0) AS EXIST_UNITS  FROM   TRANSFER ");
            sbQueryString.Append(" WHERE QTY>NVL(QTY_OUT,0)  AND (TR_TYPE='D') AND (REG_BK_I = '" + regObj.FundCode.ToString() + "') AND (REG_BR_I = '" + regObj.BranchCode.ToString() + "') AND (REG_NO_I = " + regObj.RegNumber.ToString() + ") ");
        ...
        public decimal getTotalSaleUnitBalanceWithDate(UnitHolderRegistration regObj, string transdate)
        {
            DataTable dtTotalSaleUnitBalance = new DataTable();
            decimal totalSaleUnitBalance = 0;
            StringBuilder sbQueryString = new StringBuilder();
            ...
            sbQueryString.Append("SELECT SUM(DECODE(SL_TYPE, 'SL', QTY, 0)) + SUM(DECODE(SL_TYPE, 'TI', QTY, 0)) - SUM(DECODE(SL_TYPE, 'TO', QTY, 0)) - SUM(DECODE(SL_TYPE, 'RP', QTY, 0)) AS TOTAL_BALANCE ");
            sbQueryString.Append(" FROM (SELECT   REG_BK, REG_BR, REG_NO, SUM(QTY) AS QTY, 'SL' AS SL_TYPE FROM SALE ");
            sbQueryString.Append(" WHERE  (REG_BK = '" + regObj.FundCode.ToString() + "') AND (REG_BR = '" + regObj.BranchCode.ToString() + "') AND (REG_NO = " + regObj.RegNumber.ToString() + ")  AND SL_DT<='"+ transdate + "'");
            sbQueryString.Append(" GROUP BY REG_BK, REG_BR, REG_NO UNION ALL ");

            sbQueryString.Append(" SELECT REG_BK, REG_BR, REG_NO, SUM(QTY) AS QTY, 'RP' AS SL_TYPE  FROM REPURCHASE ");
            sbQueryString.Append(" WHERE  (REG_BK = '" + regObj.FundCode.ToString() + "') AND (REG_BR = '" + regObj.BranchCode.ToString() + "') AND (REG_NO = " + regObj.RegNumber.ToString() + ")  AND REP_DT<='" + transdate + "'");
            sbQueryString.Append(" GROUP BY REG_BK, REG_BR, REG_NO UNION ALL ");

            sbQueryString.Append(" SELECT  REG_BK_I, REG_BR_I, REG_NO_I, SUM(QTY) AS QTY, 'TI' AS SL_TYPE  FROM TRANSFER ");
            sbQueryString.Append(" WHERE (REG_BK_I = '" + regObj.FundCode.ToString() + "') AND (REG_BR_I = '" + regObj.BranchCode.ToString() + "') AND (REG_NO_I = " + regObj.RegNumber.ToString() + ") AND TR_DT<='" + transdate + "'");
            sbQueryString.Append(" GROUP BY REG_BK_I, REG_BR_I, REG_NO_I UNION ALL ");

            sbQueryString.Append(" SELECT  REG_BK_O, REG_BR_O, REG_NO_O, SUM(QTY) AS QTY, 'TO' AS SL_TYPE FROM  TRANSFER TRANSFER_1 ");
            sbQueryString.Append(" WHERE (REG_BK_O = '" + regObj.FundCode.ToString() + "') AND (REG_BR_O = '" + regObj.BranchCode.ToString() + "') AND (REG_NO_O = " + regObj.RegNumber.ToString() + ") AND TR_DT<='" + transdate + "'");
            sbQueryString.Append(" GROUP BY REG_BK_O, REG_BR_O, REG_NO_O) A");
